    ...
    def next(self, transform=True, pindex=-1, tr_idx=None):
        ...
        if self.trrosetta_base != None:
            if tr_idx != None:  # testing
                sub_s = glob.glob(
                    self.trrosetta_base + "%s/new_0/naive_rolled.npz" % pname
                )
                sub_s.extend(
                    glob.glob(self.trrosetta_base + "%s/sub_0/naive.npz" % pname)
                )
                base_dist = sub_s[0]
            else:  # training
                sub_s = glob.glob(
                    self.trrosetta_base + "%s/sub_[0-2]/naive.npz" % pname
                )
                sub_s.extend(
                    glob.glob(self.trrosetta_base + "%s/sub_[0-2]/templ.npz" % pname)
                )
                sub_s.extend(
                    glob.glob(
                        self.trrosetta_base + "%s/new_[0-2]/naive_rolled.npz" % pname
                    )
                )
                sub_s.extend(
                    glob.glob(
                        self.trrosetta_base + "%s/new_[0-2]/templ_rolled.npz" % pname
                    )
                )
        ...
        orientations = np.stack(
            [data["omega6d"], data["theta6d"], data["phi6d"]], axis=-1
        )
        orientations = np.concatenate(
            [np.sin(orientations), np.cos(orientations)], axis=-1
        )
        maps = data["maps"]
        tbt = data["tbt"].T
        sep = seqsep(psize)
        ...
        if self.distribution:
            return (
                ...
                np.concatenate(
                    [maps, orientations, sep, distogram_pred, dist], axis=-1
                ),
                (estogram, native),
            )
            ...
            if self.trrosetta_base != None:
                return (
                    ...
                    np.concatenate(
                        [tbt, maps, orientations, sep, distogram_pred], axis=-1
                    ).astype(np.float32),
                    (estogram, native),
                )
                ...
                return (
                    ...
                    np.concatenate([angles, obt, prop], axis=-1).astype(np.float32),
                    np.concatenate(
                        [tbt, maps, euler, orientations, sep], axis=-1
                    ).astype(np.float32),
                    (estogram, native),
                )
