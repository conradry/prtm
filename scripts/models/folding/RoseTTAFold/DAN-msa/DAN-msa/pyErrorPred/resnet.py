...
def resnet_block(
    ...
    if require_bn:
        _input = tf.layers.batch_normalization(
            _input, training=isTraining, center=True, scale=True, axis=bn_axis
        )
    if require_in:
        _input = tf.contrib.layers.instance_norm(_input)

    _input = activation(_input)
    _input = tf.layers.conv2d(
        _input, filters=down_channel, kernel_size=(1, 1), data_format=data_format
    )
    ...
    _input = activation(_input)
    _input = tf.layers.conv2d(
        _input, filters=channel, kernel_size=(1, 1), data_format=data_format
    )
    return _input
