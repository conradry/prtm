...
def getEulerOrientation(pose):
    ...
    for i in range(1, pose.size() + 1):
        for j in range(1, pose.size() + 1):
            if i == j:
                continue
            rt6 = pyrosetta.rosetta.core.scoring.motif.get_residue_pair_rt6(
                pose, i, pose, j
            )
...
def getEnergy(p, scorefxn):
    ...
    for i in range(1, nres + 1):
        res_energy_no_two_body_z[i - 1] = (
            p.energies().residue_total_energies(i).dot(onebody_weights)
        )
        ...
        for j in range(1, nres + 1):
            ...
            if edge is None:
                ...
                res_pair_energy_z[i - 1][j - 1] = edge.fill_energy_map().dot(
                    scorefxn.weights()
                )
