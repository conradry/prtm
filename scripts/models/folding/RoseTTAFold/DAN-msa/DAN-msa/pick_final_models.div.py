...
def lrQres2CAdev(estogram, mask, minseqsep=13):
    ...
    for i in range(nres):
        for j in range(nres):
            if abs(i - j) < minseqsep:
                continue  ## up to 3 H turns
            if mask[i][j] > 0.1:
                contacts.append((i, j, mask[i][j]))
...
def calc_lddt_dist(args):
    ...
    #
    lddt_1 = float(
        os.popen("%s/lddt/lddt -c %s %s | grep Glob" % (script_dir, pose_i, pose_j))
        .readlines()[-1]
        .split()[-1]
    )
    lddt_2 = float(
        os.popen("%s/lddt/lddt -c %s %s | grep Glob" % (script_dir, pose_j, pose_i))
        .readlines()[-1]
        .split()[-1]
    )
...
for score, pdb in rep_s:
    ...
    #
    modelQ_dat.append(
        "model_%d     %.4f   %s" % (i_model, score, os.path.realpath(pdb))
    )
