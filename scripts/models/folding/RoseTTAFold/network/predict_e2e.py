...
import util
from ffindex import *
from kinematics import c6d_to_bins2, xyz_to_c6d, xyz_to_t2d
...
class Predictor:
    ...
    def predict(
        self, a3m_fn, out_prefix, hhr_fn=None, atab_fn=None, window=150, shift=75
    ):
            ...
            if L > window * 2:
                ...
                for i in range(ngrids):
                    for j in range(i, ngrids):
                        ...
                        input_msa = msa[:, :, sel]
                        mask = (
                            torch.sum(input_msa == 20, dim=-1) < 0.5 * sel.sum()
                        )  # remove too gappy sequences
                ...
                with torch.cuda.amp.autocast():
                    xyz, lddt = self.model(
                        node_s, seq, idx_pdb, prob_s=prob_in, refine_only=True
                    )
            else:
                ...
                with torch.cuda.amp.autocast():
                    logit_s, _, xyz, lddt = self.model(
                        msa, seq, idx_pdb, t1d=t1d, t2d=t2d
                    )
                prob_s = list()
        ...
        self.write_pdb(
            seq[0], xyz[0], idx_pdb[0], Bfacts=lddt[0], prefix="%s_init" % (out_prefix)
        )
...
def get_args():
    ...
    parser.add_argument("--atab", default=None, help="HHsearch output file (atab file)")
    parser.add_argument(
        "--db",
        default="%s/pdb100_2021Mar03/pdb100_2021Mar03" % script_dir,
