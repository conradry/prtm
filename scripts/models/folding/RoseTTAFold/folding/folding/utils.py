    ...
    for a, b, p, p_12 in zip(i, j, prob, prob_12):
        ...
        rst["dist"].append(
            [
                ...
                p,
                p_12,
                rosetta.core.scoring.constraints.AtomPairConstraint(ida, idb, spline),
            ]
        )
    ...
    for a, b, p in zip(i, j, prob):
        ...
        rst["omega"].append(
            [
                ...
                b,
                p,
                rosetta.core.scoring.constraints.DihedralConstraint(
                    id1, id2, id3, id4, spline
                ),
            ]
        )
    ...
    for a, b, p in zip(i, j, prob):
        ...
        rst["phi"].append(
            [
                ...
                b,
                p,
                rosetta.core.scoring.constraints.AngleConstraint(id1, id2, id3, spline),
            ]
        )
...
def set_predicted_dihedral(pose, phi, psi, omega):
    nbins = phi.shape[1]
    bins = np.linspace(-180.0, 180.0, nbins + 1)[:-1] + 180.0 / nbins
...
def add_rst(
    pose, rst, sep1, sep2, params, nogly=False, use_orient=None, pcut=None, p12_cut=0.0
):
    if use_orient == None:
    ...
    if nogly == True:
        ...
        if use_orient:
            omega_r = [
                r
                for a, b, p, r in rst["omega"]
                if abs(a - b) >= sep1
                and abs(a - b) < sep2
                and seq[a] != "G"
                and seq[b] != "G"
                and p >= pcut + P_ADD_OMEGA
            ]  # 0.5
            theta_r = [
                r
                for a, b, p, r in rst["theta"]
                if abs(a - b) >= sep1
                and abs(a - b) < sep2
                and seq[a] != "G"
                and seq[b] != "G"
                and p >= pcut + P_ADD_THETA
            ]  # 0.5
            phi_r = [
                r
                for a, b, p, r in rst["phi"]
                if abs(a - b) >= sep1
                and abs(a - b) < sep2
                and seq[a] != "G"
                and seq[b] != "G"
                and p >= pcut + P_ADD_PHI
            ]  # 0.6
    else:
        dist_r = [
            r
        ...
        if use_orient:
            omega_r = [
                r
                for a, b, p, r in rst["omega"]
                if abs(a - b) >= sep1 and abs(a - b) < sep2 and p >= pcut + P_ADD_OMEGA
            ]
            theta_r = [
                r
                for a, b, p, r in rst["theta"]
                if abs(a - b) >= sep1 and abs(a - b) < sep2 and p >= pcut + P_ADD_THETA
            ]
            phi_r = [
                r
                for a, b, p, r in rst["phi"]
                if abs(a - b) >= sep1 and abs(a - b) < sep2 and p >= pcut + P_ADD_PHI
            ]  # 0.6
