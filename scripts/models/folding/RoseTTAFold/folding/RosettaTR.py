...
def main():
    ...
    )  # detect disulfide bonds based on Cb-Cb distance (CEN mode) or SG-SG distance (FA mode)
    init_cmd.append(
    ...
    if not os.path.exists("%s_before_relax.pdb" % (".".join(args.OUT.split(".")[:-1]))):
        ...
        for run in range(params["NRUNS"]):
            ...
            sf.set_weight(
                ...
                rsr_dist_weight.setdefault(run, 1.0),
            )
            sf.set_weight(
                rosetta.core.scoring.dihedral_constraint,
                rsr_orient_weight.setdefault(run, 0.5),
            )
            sf.set_weight(
            ...
            if args.save_chk:
                pose.dump_pdb(
                    "%s_run%d_init.pdb" % (".".join(args.OUT.split(".")[:-1]), run)
                )
            ...
            if args.mode == 0:
                ...
                if args.save_chk:
                    pose.dump_pdb(
                        "%s_run%d_mode%d_step%d.pdb"
                        % (".".join(args.OUT.split(".")[:-1]), run, args.mode, 0)
                    )
                ...
                if args.save_chk:
                    pose.dump_pdb(
                        "%s_run%d_mode%d_step%d.pdb"
                        % (".".join(args.OUT.split(".")[:-1]), run, args.mode, 1)
                    )
                ...
                if args.save_chk:
                    pose.dump_pdb(
                        "%s_run%d_mode%d_step%d.pdb"
                        % (".".join(args.OUT.split(".")[:-1]), run, args.mode, 2)
                    )
        ...
        pose0 = pose_from_file(
            "%s_before_relax.pdb" % (".".join(args.OUT.split(".")[:-1]))
        )
        #
        print("to centroid")
    ...
    if args.fastrelax == True:
        ...
        relax_round1 = rosetta.protocols.relax.FastRelax(
            sf_fa_round1, "%s/data/relax_round1.txt" % scriptdir
        )
        ...
        relax_round2 = rosetta.protocols.relax.FastRelax(
            sf_fa, "%s/data/relax_round2.txt" % scriptdir
        )
