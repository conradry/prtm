...
def parse_args():
    ...
    parser.add_argument("--namespace", type=str, default="hiv", help="Model namespace")
    ...
    parser.add_argument(
        "--n-epochs", type=int, default=4, help="Number of training epochs"
    )
...
def process(args, fnames, meta_fnames):
    ...
    for fname in fnames:
        for record in SeqIO.parse(fname, "fasta"):
            ...
            if args.namespace == "hiva" and (not meta["subtype"].startswith("A")):
                continue
            if "X" in record.seq:
                continue
...
def evo_env(args, model, seqs, vocabulary, namespace="hiv_env"):
    ...
    try:
        ...
        seqs = populate_embedding(
            args, model, seqs, vocabulary, namespace="hiv", use_cache=True
        )
    ...
    sc.pl.umap(
        adata, color="pseudotime", cmap="inferno", save=f"_{namespace}_pseudotime.png"
    )
