...
def benchmark_temporal(protein, setting, label):
    ...
    return ss.spearmanr(adata.obs[label], adata.obs["pseudotime"], nan_policy="omit")[0]
...
def benchmark_class(protein, setting, labels):
    ...
    if len(set(classes)) > 2:
        return (ss.spearmanr(classes, pseudotimes, nan_policy="omit")[0], "spearmanr")
...
if __name__ == "__main__":
    ...
    for protein in proteins:
        for setting in settings:
            ...
            if protein in temporal_benchmarks:
                ...
                data.append([protein, setting, "spearmanr", value])
            if protein in class_benchmarks:
                value, score_type = benchmark_class(
                    protein, setting, class_benchmarks[protein]
                )
                data.append([protein, setting, score_type, value])
    ...
    plt.axhline(0.0, color="#888888", linestyle="--")
