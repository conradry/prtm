...
def get_model(args, seq_len, vocab_size, inference_batch_size=1500):
...
def featurize_seqs(seqs, vocabulary):
    ...
    X = np.concatenate(
        [
            np.array([start_int] + [vocabulary[word] for word in seq] + [end_int])
...
def report_performance(model_name, model, vocabulary, train_seqs, test_seqs):
    ...
    tprint(
        "Model {}, train cross entropy: {}".format(
            model_name, cross_entropy(logprob, len(lengths_train))
        )
    )
    X_test, lengths_test = featurize_seqs(test_seqs, vocabulary)
    logprob = model.score(X_test, lengths_test)
    tprint(
        "Model {}, test cross entropy: {}".format(
            model_name, cross_entropy(logprob, len(lengths_test))
        )
    )
...
def train_test(args, model, seqs, vocabulary, split_seqs=None):
    ...
    if args.test:
        report_performance(args.model_name, model, vocabulary, train_seqs, val_seqs)


def batch_train(args, model, seqs, vocabulary, batch_size=5000, verbose=True):
    assert args.train
    ...
    if verbose:
        tprint("Traing seq batch size: {}, N batches: {}".format(batch_size, n_batches))
    ...
    for epoch in range(n_epochs):
        ...
        fname_prefix = "target/{0}/checkpoints/{1}/{1}_{2}".format(
            args.namespace, args.model_name, args.dim
        )

        if epoch == 0:
            os.rename(
                "{}-01.hdf5".format(fname_prefix), "{}-00.hdf5".format(fname_prefix)
            )
        else:
            os.rename(
    ...
    os.rename("{}-00.hdf5".format(fname_prefix), "{}-01.hdf5".format(fname_prefix))


def embed_seqs(
    args, model, seqs, vocabulary, use_cache=False, verbose=True, namespace=None
):
    ...
    if use_cache:
        ...
        embed_fname = "target/{}/embedding/{}_{}.npy".format(
            args.namespace, args.model_name, args.dim
        )
    else:
        embed_fname = None
...
def populate_embedding(args, model, seqs, vocabulary, use_cache=False, namespace=None):
    if namespace is None:
        namespace = args.namespace

    if use_cache:
        mkdir_p("target/{}/embedding".format(namespace))
        embed_prefix = "target/{}/embedding/{}_{}".format(
            namespace, args.model_name, args.dim
        )
    ...
    for batchi in range(n_batches):
        ...
        # Embed the sequences.
        seqs_batch = embed_seqs(args, model, seqs_batch, vocabulary, use_cache=False)
        if use_cache:
            X_embed = []
...
def predict_sequence_prob(args, seq_of_interest, vocabulary, model, verbose=False):
...
def analyze_comb_fitness(
    ...
    seqs_fitness = {
        seq: seqs_fitness[(seq, strain_i)]
        for seq, strain_i in seqs_fitness
        if strain_i == strain
    }

    y_pred = predict_sequence_prob(args, wt_seq, vocabulary, model, verbose=verbose)
    ...
    base_embedding = embed_seqs(
        args, model, {wt_seq: [{}]}, vocabulary, use_cache=False, verbose=False
    )[wt_seq][0]["embedding"]
    ...
    for batchi in range(n_batches):
        ...
        seqs_fitness_batch = embed_seqs(
            args, model, seqs_fitness_batch, vocabulary, use_cache=False, verbose=False
        )
    ...
    df = pd.DataFrame(
        data,
        columns=["strain", "fitness", "preference", "predicted", "sem_change", "cscs"],
    )

    print("\nStrain: {}".format(strain))
    print("\tGrammaticality correlation:")
    print(
        "\t\tSpearman r = {:.4f}, P = {:.4g}".format(
            *ss.spearmanr(df.preference, df.predicted)
        )
    )
    print(
        "\t\tPearson rho = {:.4f}, P = {:.4g}".format(
            *ss.pearsonr(df.preference, df.predicted)
        )
    )
    ...
    print(
        "\t\tSpearman r = {:.4f}, P = {:.4g}".format(
            *ss.spearmanr(df.preference, df.sem_change)
        )
    )
    print(
        "\t\tPearson rho = {:.4f}, P = {:.4g}".format(
            *ss.pearsonr(df.preference, df.sem_change)
        )
    )
    ...
    plt.savefig(
        "figures/combinatorial_fitness_grammar_{}_{}.png".format(
            args.namespace, strain
        ),
        dpi=300,
    )
    plt.close()
    ...
    plt.savefig(
        "figures/combinatorial_fitness_semantics_{}_{}.png".format(
            args.namespace, strain
        ),
        dpi=300,
    )
...
def analyze_semantics(
    ...
    if plot_acquisition:
        ...
        with open(ofname, "w") as of:
            ...
            for seq in seqs:
                try:
                    didx = [c1 != c2 for c1, c2 in zip(seq_to_mutate, seq)].index(True)
                    of.write(
    ...
    base_embedding = embed_seqs(
        args, model, {seq_to_mutate: [{}]}, vocabulary, use_cache=False, verbose=False
    ...
    for batchi in range(n_batches):
        ...
        prob_seqs_batch = {
            seq: deepcopy(prob_seqs[seq])
            for seq in seqs[start:end]
            if seq != seq_to_mutate
        }
        prob_seqs_batch = embed_seqs(
            args, model, prob_seqs_batch, vocabulary, use_cache=False, verbose=False
        )
    ...
    cache_fname = dirname + (
        "/analyze_semantics_{}_{}_{}.txt".format(
            plot_namespace, args.model_name, args.dim
        )
    )
    probs, changes = [], []
    with open(cache_fname, "w") as of:
        fields = ["pos", "wt", "mut", "prob", "change", "is_viable", "is_escape"]
    ...
    if plot_acquisition:
        ...
        cached_escape(
            cache_fname, beta, plot=plot_acquisition, namespace=plot_namespace
        )
...
def analyze_reinfection(
    ...
    y_pred = predict_sequence_prob(args, wt_seq, vocabulary, model, verbose=False)
    word_pos_prob = {}
    ...
    mkdir_p(dirname)
    fname = dirname + "/{}_mut_{}.txt".format(args.namespace, namespace)
    ...
    with open(fname, "w") as of:
        for mutant in mutants[n_mutations]:
            positions = [
                pos for pos in range(len(wt_seq)) if wt_seq[pos] != mutant[pos]
            ]
            ...
            embedding = embed_seqs(
                args, model, {mutant: [{}]}, vocabulary, use_cache=False, verbose=False
            )[mutant][0]["embedding"]
            sem_change = abs(base_embedding - embedding).sum()

            fields = [mut_str.rstrip(","), n_mutations, seq_prob, sem_change]
            of.write("\t".join([str(field) for field in fields]) + "\n")
    ...
    with open(fname, "a") as of:
        for seq in seqs:
            meta = seqs[seq][0]
            if meta["strain"] != "SARS-CoV-2" and "hCoV-19" not in meta["strain"]:
                continue
            if seq == wt_seq:
                continue
            ...
            fields = [mut_str.rstrip(","), len(raw_probs), seq_prob, sem_change]
            of.write("\t".join([str(field) for field in fields]) + "\n")
...
def null_combinatorial_fitness(
    ...
    mkdir_p(dirname)
    fname = dirname + "/{}_mut_{}.txt".format(args.namespace, n_mutations)
    ...
    seq_prob = {}
    for mutant in mutants[n_mutations]:
        positions = [pos for pos in range(len(wt_seq)) if wt_seq[pos] != mutant[pos]]
        assert len(positions) == n_mutations
        mut_str, raw_probs = "", []
        for pos in positions:
            word = mutant[pos]
    ...
    for _ in range(n_permutations):
        positions = np.random.choice(len(wt_seq), n_mutations, replace=False)
        mut_str, raw_probs = "", []
        for pos in positions:
