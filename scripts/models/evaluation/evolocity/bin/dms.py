...
def dms_results(fname, args, model, vocabulary):
    ...
    seq_cache[wt_seq] = np.array(
        [
            y_pred[
                i + 1,
                (vocabulary[wt_seq[i]] if wt_seq[i] in vocabulary else model.unk_idx_),
            ]
    ...
    for mut in df["variant"]:
        ...
        if aa_mut == wt_seq[pos]:
            ...
            scores_pred.append(
                likelihood_muts(
                    wt_seq, seq_mut, args, vocabulary, model, seq_cache=seq_cache
                )
            )
    ...
    for column in df.columns:
        if column.startswith("DMS"):
            ...
            if "DeepSequence" not in df or df["DeepSequence"].isnull().values.all(
                axis=0
            ):
