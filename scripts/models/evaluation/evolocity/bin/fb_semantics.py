...
def predict_sequence_prob_mask_fb(
    ...
    seqs = [seq[:c_idx] + "0" + seq[c_idx + 1 :] for c_idx in range(len(seq))]
    labels = [str(c_idx + 1) + c for c_idx, c in enumerate(seq)]
...
def fb_semantics(
    ...
    y_pred = predict_sequence_prob_fb(
        seq_to_mutate, alphabet, model, repr_layers, verbose=verbose
    )
    ...
    word2idx = {word: alphabet.all_toks.index(word) for word in alphabet.all_toks}
    ...
    base_embedding = embed_seqs_fb(
        model, [seq_to_mutate], repr_layers, alphabet, use_cache=False, verbose=False
    ...
    for batchi in range(n_batches):
        ...
        prob_seqs_batch = [seq for seq in seqs[start:end] if seq != seq_to_mutate]
    ...
    cache_fname = dirname + (
        "/analyze_semantics_{}_{}.txt".format(plot_namespace, model.model_version)
    )
    probs, changes = [], []
