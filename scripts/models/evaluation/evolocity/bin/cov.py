...
def parse_args():
    ...
    parser.add_argument("--namespace", type=str, default="cov", help="Model namespace")
    parser.add_argument("--dim", type=int, default=512, help="Embedding dimension")
    parser.add_argument(
        "--batch-size", type=int, default=500, help="Training minibatch size"
    )
    parser.add_argument(
        "--n-epochs", type=int, default=11, help="Number of training epochs"
    )
...
def parse_gisaid(entry):
    ...
    if type_id in {"bat", "canine", "cat", "env", "mink", "pangolin", "tiger"}:
        host = type_id
        country = "NA"
        continent = "NA"
    else:
...
def setup(args):
    ...
    model = get_model(args, seq_len, vocab_size, inference_batch_size=1200)
...
def seqs_to_anndata(seqs):
    ...
    for seq in seqs:
        ...
        earliest_idx = np.argmin([meta["timestamp"] for meta in seqs[seq]])
        for key in meta:
            if key == "embedding":
                continue
            if key not in obs:
                obs[key] = []
            obs[key].append([meta[key] for meta in seqs[seq]][earliest_idx])
...
def spike_evolocity(args, model, seqs, vocabulary, namespace="cov"):
    ...
    try:
        ...
        adata = adata[
            adata.obs["timestamp"] > time.mktime(dparse("2019-11-30").timetuple())
        ]
    ...
    nnan_idx = np.isfinite(adata.obs["timestamp"]) & np.isfinite(
    ...
    plt.figure()
    sns.regplot(x="timestamp", y="pseudotime", ci=None, data=adata_nnan.obs)
    plt.ylim([-0.01, 1.01])
    ...
    tprint(
        "Pseudotime-time Pearson r = {}, P = {}".format(
            *ss.pearsonr(adata_nnan.obs["pseudotime"], adata_nnan.obs["timestamp"])
        )
    )
...
if __name__ == "__main__":
    ...
    if args.evolocity:
        ...
        spike_evolocity(args, model, seqs, vocabulary, namespace=args.namespace)
