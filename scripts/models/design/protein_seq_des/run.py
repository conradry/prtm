...
def log_metrics(
    run="sampler", args=None, log=None, iteration=0, design_sampler=None, prefix=""
):
    # tensorboard logging

    # log structure / sequence metrics
    log.log_scalar(
        "{run}/{prefix}rosetta_energy".format(run=run, prefix=prefix),
        design_sampler.rosetta_energy,
    )
    log.log_scalar(
        "{run}/{prefix}seq_overlap".format(run=run, prefix=prefix),
        design_sampler.seq_overlap,
    )
    log.log_scalar(
        "{run}/{prefix}anneal_start_temp".format(run=run, prefix=prefix),
        design_sampler.anneal_start_temp,
    )
    log.log_scalar(
        "{run}/{prefix}anneal_final_temp".format(run=run, prefix=prefix),
        design_sampler.anneal_final_temp,
    )
    log.log_scalar(
        "{run}/{prefix}log_p".format(run=run, prefix=prefix),
        design_sampler.log_p_mean.item(),
    )
    log.log_scalar(
        "{run}/{prefix}chi_error".format(run=run, prefix=prefix),
        design_sampler.chi_error,
    )
    log.log_scalar(
        "{run}/{prefix}chi_rmsd".format(run=run, prefix=prefix), design_sampler.chi_rmsd
    )

    # log rosetta score terms
    for s in design_sampler.score_terms:
        log.log_scalar(
            "{run}/z_{prefix}{s}".format(run=run, prefix=prefix, s=s),
            float(design_sampler.curr_score_terms[s].mean()),
        )
...
def main():
    ...
    if not os.path.isfile(args.pdb):
        ...
        os.system(
            "wget -O {} https://files.rcsb.org/download/{}.pdb".format(
                args.pdb, args.pdb[:-4].upper()
            )
        )
    ...
    # load models
    if args.init_model != "":
        init_classifier = load_model(
            args.init_model, use_cuda=use_cuda, nic=len(common.atoms.atoms)
        )
        init_classifier.eval()
        init_classifiers = [init_classifier]
    else:
        assert not (
            args.ala and args.val
        ), "must specify either poly-alanine or poly-valine"
        if args.randomize:
            if args.ala:
    ...
    design_sampler = sampler.Sampler(
        args, classifiers, init_classifiers, log=log, use_cuda=use_cuda
    )
    ...
    log_metrics(
        args=args, log=log, iteration=0, design_sampler=design_sampler, prefix="gt_"
    )
    ...
    design_sampler.pose.dump_pdb(
        log.log_path + "/" + args.file_dir + "/" + "curr_0.pdb"
    )
        ...
        for i in tqdm(range(1, int(args.n_iters)), desc="running design"):
            ...
            if design_sampler.rosetta_energy < best_rosetta_energy:
                design_sampler.pose.dump_pdb(
                    ...
                    + args.file_dir
                    + "/"
                    + "curr_best_rosetta_energy_%s.pdb" % log.ts
                )
    ...
    # save final model
    design_sampler.pose.dump_pdb(
        log.log_path + "/" + args.file_dir + "/" + "curr_final.pdb"
    )

    np.savetxt(
        "{}/{}/logmeans.txt".format(log.log_path, args.file_dir),
        logmeans,
        delimiter=",",
    )
    np.savetxt(
        "{}/{}/rosetta_energy.txt".format(log.log_path, args.file_dir),
        rosettas,
        delimiter=",",
    )
