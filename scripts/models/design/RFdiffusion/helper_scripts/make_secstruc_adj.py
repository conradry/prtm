...
def main():
    args = get_args()
    assert (
        args.input_pdb or args.pdb_dir is not None
    ), "Need to provide either an input pdb (--input_pdb) or a path to pdbs (--pdb_dir)"
    assert not (
        args.input_pdb is not None and args.pdb_dir is not None
...
def get_args():
    ...
    parser.add_argument(
        ...
        dest="out_dir",
        required=True,
        help="need to specify an output path",
    )
