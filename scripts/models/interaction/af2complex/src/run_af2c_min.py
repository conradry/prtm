...
flags.DEFINE_string(
    "input_dir",
    None,
    "Path to a directory that " "contains unrelaxed models in the PDB format.",
)
flags.DEFINE_string(
    "output_dir", None, "Path to a directory that will " "store the relaxed models."
)
flags.DEFINE_string(
    "model_str",
    "model_",
    ...
    for target in target_lst:
        ...
        for afile in os.listdir(input_dir):
            ...
            if FLAGS.model_str in afile:
                ...
                relaxed_protein = protein.from_relaxation(
                    relaxed_pdb_str, unrelaxed_protein.residue_index
                )
                relaxed_pdb_str = protein.to_pdb(relaxed_protein)

                # Save the relaxed PDB.
                relaxed_output_path = os.path.join(
                    output_dir, f"relaxed_{unrelaxed_pdb_file}"
                )
                with open(relaxed_output_path, "w") as f:
                    f.write(relaxed_pdb_str)

                logging.info(
                    f"{target_name} relaxation done, time spent {relaxation_time:.1f} seconds, "
