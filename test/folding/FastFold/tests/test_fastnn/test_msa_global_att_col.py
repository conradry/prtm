...
def get_openfold_module_and_data():
    with torch.no_grad():
        ...
        fast_module = (
            fast_module.extra_msa_stack.blocks[0]
            .msa_stack.MSAColumnAttention.eval()
            .cuda()
        )
        target_module = (
            target_module.extra_msa_stack.blocks[0].msa_att_col.eval().cuda()
        )
...
def _test_msa_global_att_col(
    rank, world_size, chunk_size, get_openfold_module_and_data
):
