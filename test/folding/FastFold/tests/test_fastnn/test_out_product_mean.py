...
def get_openfold_module_and_data():
    with torch.no_grad():
        ...
        target_module = (
            target_module.evoformer.blocks[0].core.outer_product_mean.eval().cuda()
        )
...
def _test_out_product_mean(
    ...
    assert (
        error < 1e-5
    ), f"Test failed at chunk size: {chunk_size}, inplace: {inplace}. The position dif is {error}"
