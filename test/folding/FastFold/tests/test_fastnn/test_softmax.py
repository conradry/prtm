import torch
from fastfold.model.fastnn.kernel import fused_softmax, softmax
...
def _test_softmax_core():
    ...
    for seq_ in test_seq_:
        for dtype in test_dtype:
            sample_input = (
                torch.rand(batch_, chunk_, head_, seq_, seq_)
            ...
            sample_bias = (
                torch.rand(batch_, 1, head_, seq_, seq_)
                .to(device=test_device, dtype=dtype)
                .requires_grad_(True)
            )
            ...
            torch_out = torch.nn.functional.softmax(
                sample_input + sample_mask_torch + sample_bias, dim=-1
            )

            fastnn_out = fused_softmax(
                sample_input_fastnn, sample_mask_fastnn, sample_bias_fastnn
            )

            fwd_fastnn_error = torch.max(torch.abs(torch_out - fastnn_out)).cpu().item()
            assert (
                fwd_fastnn_error < tolerance_eps[dtype]
            ), f"fastnn fwd kernel error when {seq_} {dtype}"
            ...
            assert (
                grad_input_error < tolerance_eps[dtype]
            ), f"fastnn bwd kernel error when {seq_} {dtype}"

            grad_bias_error = (
                torch.max(torch.abs(sample_bias.grad - sample_bias_fastnn.grad))
                .cpu()
                .item()
            )
            assert (
                grad_bias_error < tolerance_eps[dtype]
            ), f"fastnn bwd kernel error when {seq_} {dtype}"
