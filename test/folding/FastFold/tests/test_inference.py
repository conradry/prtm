...
def test_state_dict(world_size, chunk_size, inplace, get_openfold_module_and_data):
    run_func = partial(
        ...
        inplace=inplace,
        model=get_openfold_module_and_data,
    )
...
def inference(chunk_size, inplace, get_openfold_module_and_data):
    ...
    fastmodel.structure_module.default_frames = (
        fastmodel.structure_module.default_frames.cuda()
    )
    ...
    fastmodel.structure_module.lit_positions = (
        fastmodel.structure_module.lit_positions.cuda()
    )
    ...
    pos_dif = torch.max(
        torch.abs(fastout["final_atom_positions"] - out["final_atom_positions"].cuda())
    )
    assert (
        pos_dif < 5e-4
    ), f"Test failed at chunk size: {chunk_size}, inplace: {inplace}. The position dif is {pos_dif}"
