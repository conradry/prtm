...
import numpy as np
import torch
from prtm.models.openfold.data.data_transforms import (
    MSA_FEATURE_NAMES,
...
features_fpath = os.path.join(os.path.dirname(__file__), "test_data/features.pkl")
sample_feats_fpath = os.path.join(
    os.path.dirname(__file__), "test_data/sample_feats.pickle.gz"
)


def test_make_seq_mask():
...
def test_correct_msa_restypes():
    with open(features_fpath, "rb") as file:
...
def test_nearest_neighbor_clusters():
    with gzip.open(sample_feats_fpath, "rb") as f:
...
def test_make_atom14_masks():
    with gzip.open(sample_feats_fpath, "rb") as file:
        features = pickle.load(file)
