...
import numpy as np
import proteome.models.folding.openfold.data.data_transforms as data_transforms
...
@skip_unless_alphafold_installed()
def test_pseudo_beta_fn_compare():
    ...
    assert torch.max(torch.abs(out_gt_pos - out_repro_pos)) < consts.eps
    assert torch.max(torch.abs(out_gt_mask - out_repro_mask)) < consts.eps


@skip_unless_alphafold_installed()
def test_atom37_to_torsion_angles_compare():
    ...
    assert torch.mean(torch.abs(out_gt["torsion_angles_sin_cos"] - tasc)) < 0.01
    assert torch.mean(torch.abs(out_gt["alt_torsion_angles_sin_cos"] - atasc)) < 0.01
    assert torch.max(torch.abs(out_gt["torsion_angles_mask"] - tam)) < consts.eps


@skip_unless_alphafold_installed()
def test_atom37_to_frames_compare():
    ...
    for k, v in out_gt.items():
        assert torch.max(torch.abs(out_gt[k] - out_repro[k])) < consts.eps


def test_torsion_angles_to_frames_shape():
    batch_size = 2
    n = 5
    ...
    assert frames.shape == (batch_size, n, 8)


@skip_unless_alphafold_installed()
def test_torsion_angles_to_frames_compare():
    ...
    assert torch.max(torch.abs(transforms_gt - transforms_repro) < consts.eps)


def test_frames_and_literature_positions_to_atom14_pos_shape():
    ...
    assert xyz.shape == (batch_size, n_res, 14, 3)


@skip_unless_alphafold_installed()
def test_frames_and_literature_positions_to_atom14_pos_compare():
