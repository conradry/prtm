...
def test_structure_module_shape():
    ...
    assert out["frames"].shape == (no_layers, batch_size, n, 7)
    assert out["angles"].shape == (no_layers, batch_size, n, no_angles, 2)
    assert out["positions"].shape == (no_layers, batch_size, n, 14, 3)


def test_structure_module_transition_shape():
    batch_size = 2
    n = 5
    c = 7
    num_layers = 3
...
@skip_unless_alphafold_installed()
def test_structure_module_compare():
    ...
    assert torch.mean(torch.abs(out_gt - out_repro)) < 0.05
...
def test_ipa_compare():
    ...
    ipa_params = fetch_alphafold_module_weights(
        "alphafold/alphafold_iteration/structure_module/"
...
def test_shape():
    ...
    assert a.shape == (batch_size, n, no_angles, 2)
