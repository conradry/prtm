...
def parse_hhr(filename, ffindex, idmax=105.0):
    ...
    with open(filename, "r") as hhr:
        ...
        start = [
            i for i, l in enumerate(lines) if l and l[0] == ">"
        ]  # and l[1:].strip() in label_set]
        ...
        for idx, i in enumerate(start):
            ...
            # get hit statistics
            p, e, s, _, seqid, sim, _, neff = [
                float(s) for s in re.sub("[=%]", " ", lines[i + 1]).split()[1::2]
            ]
            ...
            matches = np.array(
                [
                    [q - 1, t - 1, s - 1, c - 1]
                    for q, t, s, c in zip(qj, tj, simlr, confd)
                    if s > 0
                ]
            )
...
def parse_pdb(filename):
    ...
    N = np.array(
        [
            ...
            for l in lines
            if l[:4] == "ATOM" and l[12:16].strip() == "N"
        ]
    )
    Ca = np.array(
        [
    ...
    C = np.array(
        [
            [float(l[30:38]), float(l[38:46]), float(l[46:54])]
            for l in lines
            if l[:4] == "ATOM" and l[12:16].strip() == "C"
        ]
    )
