...
from prtm.models.rosettafold.config import TRFoldConfig
...
class TRFold:
    ...
    def fold(self, xyz, pred, batch=32, lr=0.8, nsteps=100):
        ...
        csto = -torch.log(po[0:36, i_s, j_s] / (1.0 / 36))  # omega and theta
        cstt = -torch.log(pt[0:36, i_a, j_a] / (1.0 / 36))  # are almost uniform
        ...
        # smoothed restraints
        cstd, csto, cstt, cstp = [
            nn.functional.conv1d(cst.T.unsqueeze(1), self.sg)[:, 0]
        ...
        for step in range(nsteps):
            ...
            co = coefo[ko, obin]
            losso = (
                co[:, 0] + co[:, 1] * do + co[:, 2] * do**2 + co[:, 3] * do**3
            ).view(
                batch, -1
            )  # .mean(1)
            ...
            ct = coeft[kt, tbin]
            losst = (
                ct[:, 0] + ct[:, 1] * dt + ct[:, 2] * dt**2 + ct[:, 3] * dt**3
            ).view(
                batch, -1
            )  # .mean(1)
            ...
            cp = coefp[kt, pbin]
            lossp = (
                cp[:, 0] + cp[:, 1] * dp + cp[:, 2] * dp**2 + cp[:, 3] * dp**3
            ).view(
                batch, -1
            )  # .mean(1)
