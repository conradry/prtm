...
from prtm.models.rosettafold.performer_pytorch import SelfAttention
...
class TiedMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, return_att=False):
        ...
        q = (
            self.to_query(query)
            .view(B, N, L, self.heads, self.d_k)
            .permute(0, 1, 3, 2, 4)
            .contiguous()
        )  # (B, N, h, l, k)
        k = (
...
class DirectMultiheadAttention(nn.Module):
    ...
    def forward(self, src, tgt):
        B, N, L = tgt.shape[:3]
        attn_map = F.softmax(self.proj_pair(src), dim=1).permute(
            0, 3, 1, 2
        )  # (B, h, L, L)
...
class MaskedDirectMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        q = (
            self.to_query(query).view(batch, L, self.heads, -1).permute(0, 2, 1, 3)
        )  # (B, h, L, -1)
        k = (
            self.to_key(key).view(batch, L, self.heads, -1).permute(0, 2, 1, 3)
        )  # (B, h, L, -1)
        v = (
            self.to_value(value)
            .view(batch, N, L, self.heads, -1)
            .permute(0, 3, 1, 2, 4)
        )  # (B, h, N, L, -1)
        #
        q = q * self.scaling
