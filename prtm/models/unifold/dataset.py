...
class UnifoldDataset(Dataset):
    def __init__(
        ...
        self.batch_size = (
            args.batch_size * get_data_parallel_world_size() * args.update_freq[0]
        )
        self.data_len = (
    ...
    def collater(samples):
        ...
        return collate_dict(samples, dim=1)
...
class UnifoldMultimerDataset(Dataset):
    def __init__(
        self,
        args: mlc.ConfigDict,
        seed: int,
    ...
    def __getitem__(self, idx):
        ...
        if is_distillation:
            ...
            if pdb_id in self.pdb_assembly and self.mode == "train":
                ...
                symmetry_operations = []
                for cid, op in zip(
                    self.pdb_assembly[pdb_id]["chains"],
                    self.pdb_assembly[pdb_id]["opers"],
                ):
                    lid = pdb_id + "_" + cid
                    if lid not in self.inverse_multi_label:
                        logger.warning(
                            f"sequence id of {lid} cannot be found. chain ignored in the assembly."
                        )
                    else:
    ...
    def collater(samples):
        ...
        try:
            feats = collate_dict(feats, dim=1)
        except:
