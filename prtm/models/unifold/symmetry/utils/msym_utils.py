...
def arr_to_elems(arr: np.ndarray, name: str = "C") -> List[m.Element]:
    assert (
        len(arr.shape) == 2 and arr.shape[-1] == 3
    ), f"invalid points shape {arr.shape}"
    return [m.Element(name=name, coordinates=r) for r in arr]


def get_rotate_theta_axis(
    pts: np.ndarray,
...
def collect_axis_with_fac(
    list_theta_axis: List[Tuple[float, np.ndarray]], angle_fac, debug=False
) -> List[np.ndarray]:
    """
...
def combine_axes(
    list_list_axis: List[List[np.ndarray]],
    ...
    def search(state: Tuple, reverse: Tuple):
        ...
        list_ret_axis = [
            list_list_axis[i][s] * r for i, (s, r) in enumerate(zip(state, reverse))
        ]
        if depth == t_len:
...
def au_with_axes_msym(
    labels: List[LabelType], symmetry: str, centers: List[np.ndarray], debug=False
    ...
    list_list_axis = [
        collect_axis_with_fac(list_theta_axis, 2 * np.pi / n_sym, debug=debug)
        for n_sym in list_num_sym
    ]
    ...
    for num_sym, axis in zip(list_num_sym, list_ret_axis):
        succeed, groups, delta = recover_groups_by_axis(
            centers, entity_ids, num_sym, axis, debug=False, strict_mod=False
        )
