...
from .geometry_utils import LabelType, calc_distance_map
...
def get_pair_distance_set_dict(
    labels: List[LabelType], centers: List[np.ndarray]
) -> Dict[str, Sequence]:
    """
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            pair_distance_set_dict.setdefault(key, []).append(
                center_distance_matrix[i, j]
            )
