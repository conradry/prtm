...
from prtm.models.unifold.symmetry.au_span import get_pair_distance_set_dict, span_au
...
from prtm.models.unifold.symmetry.inertia import (
    best_inertia_gap_id,
    get_ref_idx,
    inertia_gap,
)
from prtm.models.unifold.symmetry.msym_utils import au_with_axes_msym
from prtm.models.unifold.symmetry.quaternion import quaternion_rotate


def masked_mean_pos(
    pos: np.ndarray, mask: np.ndarray, strict: bool = True, axis: Union[int, Tuple] = -2
):
    """
    pos: [*, 3]
    mask: [*]
...
def rodrigues(a, b, eps=1e-6, normalized: bool = False):
    ...
    c = np.dot(a, b)
    if np.abs(1.0 + c) < eps:  # cosa = -1, i.e. a, b opposite
        return -np.eye(3)
    vx = np.array([[0, -v[2], v[1]], [v[2], 0, -v[0]], [-v[1], v[0], 0]])
    r = np.eye(3) + vx + (1.0 / (1.0 + c)) * vx @ vx
    # assert np.all(r @ a - b < eps)
    return r


def get_centers(
    labels: List[LabelType], entity_all_atom_mask=None, removal=True
) -> List[np.ndarray]:
    """
    Get unit centers.
...
def get_entity_all_atom_mask(labels: List[LabelType]) -> Dict[str, Any]:
    ...
    for entity_id, ls in entity_group_labels:
        entity_all_atom_mask[entity_id] = reduce(
            lambda x, y: x * y, map(lambda x: x["all_atom_mask"], ls)
        )
...
def normalize_update_labels(labels: List[LabelType], use_inertia=False, debug=False):
    ...
    for label in labels:
        label["temp_all_atom_mask"] = entity_all_atom_mask[label["entity_id"][0]]
    entity_aai = {
        k: np.argwhere(aam[:, ca_idx] > 0.5)[:, 0]
        for k, aam in entity_all_atom_mask.items()
    }
    ...
    if use_inertia:
        ...
        for i, label in enumerate(labels):
            ...
            list_ref_vec = [
                ...
                xs[aai[-int(aai.shape[0] / 2)][0], aai[-int(aai.shape[0] / 2)][1]]
                - xs[aai[0][0], aai[0][1]],
        ...
        for i, label in enumerate(labels):
            ...
            list_ref_vec = [
                xs[aai[v], ca_idx, :] - xs[aai[u], ca_idx, :]
                for u, v in get_ref_idx(len(aai))
            ]
            main_ref_vecs.append(main_ref_vec)
            list_ref_vecs.append(list_ref_vec)
            entity_ref_cos.setdefault(label["entity_id"][0], []).append(
                [
                    np.abs(np.dot(main_ref_vec, ref_vec))
                    / (np.linalg.norm(ref_vec) * np.linalg.norm(main_ref_vec))
        ...
        for i, label in enumerate(labels):
            main_ref_vec, ref_vec = (
                main_ref_vecs[i],
                list_ref_vecs[i][entity_ref_id[label["entity_id"][0]]],
            )
            ...
            label["format"][:, 1], label["format"][:, 2], label["format"][:, 3] = (
                f1,
                f2,
                f3,
            )  # orientation
...
def get_au_axes(
    labels: List[LabelType], symmetry: str, use_msym=False, debug=False
    ...
    if use_msym:
        au_labels, axes = au_with_axes_msym(
            labels,
            symmetry,
            get_centers(labels, get_entity_all_atom_mask(labels), removal=True),
            debug=debug,
        )
    else:
...
def translate_rotate_labels(
    labels: List[LabelType], axes: Optional[np.ndarray], symmetry: str, debug=False
):
    """
    ...
    if symmetry.startswith("C"):
        if axes is not None:
            axis = axes[0, :]
            rot_mat = rodrigues(axis / np.linalg.norm(axis), np.array([0.0, 0.0, 1.0]))
            for label in labels:
...
def score_au_symmetry_match(
    all_labels: List[LabelType],
    au_labels: List[LabelType],
    symm_op: np.ndarray,
    thresholds: Sequence[int] = (3, 5, 8, 13, 21),
    ...
    """
    if len(all_labels) == 1:
        return 1.0, [1.0 for _ in thresholds]
    ...
    delta_dis = np.concatenate(
        [np.abs(np.array(src_dis[k]) - np.array(tgt_dis[k])) for k in src_dis.keys()]
    )
    hierarchy_passed_points = [delta_dis < i for i in thresholds]
    scores = [
        np.sum(passed_points) / len(passed_points)
        for passed_points in hierarchy_passed_points
    ]
    score = sum(scores) / len(scores)
    return score, scores


def get_normalized_au(
    labels: List[LabelType],
    symmetry: Optional[str],
    accept_score=0.6,
    rotate_fail_tolerate=True,
    debug=False,
    ...
    try:
        ...
        switch_symmetry()
        score, _ = score_au_symmetry_match(
            labels, au_labels, symm_op=get_transform(symmetry)
        )
        ...
        try:
            ...
            if score < accept_score:
                raise RotatingFailError(
                    f"Too low rotation score: {score} < {accept_score}"
                )
        ...
        try:
            former_au_labels, former_handle_symmetry, former_score = (
                au_labels,
                handle_symmetry,
                score,
            )
            ...
            if score < former_score:
                au_labels, handle_symmetry, score = (
                    former_au_labels,
                    former_handle_symmetry,
                    former_score,
                )
            if score < accept_score:
                raise RotatingFailError(
