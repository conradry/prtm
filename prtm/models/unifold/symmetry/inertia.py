...
def inertia_gap_by_ref_vec(
    inertia: np.ndarray, ref_vec: np.ndarray
...
def inertia_gap(
    inertia: np.ndarray, list_ref_vec: List[np.ndarray]
) -> Tuple[List[np.ndarray], List[float]]:
    list_ref_inertia_gap = [
        inertia_gap_by_ref_vec(inertia, ref_vec) for ref_vec in list_ref_vec
    ]
...
def best_inertia_gap_id(
    list_inertia_gaps: List[Tuple[List[np.ndarray], List[float]]]
) -> int:
