...
class UFSymmetry(AlphaFold):  # inherit the main model. alterations implemented here.
    ...
    def iteration_evoformer(self, feats, m_1_prev, z_prev, x_prev):
        ...
        m, z, pr_m = self.input_embedder(
            feats["target_feat"], feats["msa_feat"], pr_feat
        )
        ...
        # pad pr mask
        pad_fn = lambda msk: F.pad(msk, (1, 0), "constant", 1.0)
    ...
    def iteration_evoformer_structure_module(
        ...
        for ensemble_no in range(num_ensembles):
            ...
            (
                ...
                m_1_prev_emb,
                z_prev_emb,
            ) = self.iteration_evoformer(feats, m_1_prev, z_prev, x_prev)
            z += z0
            s += s0
            del z0, s0
        if num_ensembles > 1:
        ...
        global_center = global_center.repeat(
            *((1,) * len(global_center.shape[:-2])), 37, 1
        )
    ...
    def forward(self, batch, expand=True):
        ...
        if expand:
            ...
            outputs["expand_final_atom_positions"] = symmetry_output[
                "expand_final_atom_positions"
            ]
            outputs["expand_final_atom_mask"] = symmetry_output[
                "expand_final_atom_mask"
            ]
