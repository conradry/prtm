...
class StructureConditioner(nn.Module):
    def __init__(self, d_in, d_model, n_layers, kernel_size, r, dropout=0.0):
        super().__init__()
        self.embedder = ByteNet2d(
            d_in, d_model, n_layers, kernel_size, r, dropout=dropout
        )
        self.attention = Attention2d(d_model)

    def forward(self, x, input_mask=None):
        return self.attention(
            self.embedder(x, input_mask=input_mask), input_mask=input_mask
        )
...
class StructureConditionedBytenet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.conditioner = StructureConditioner(
            d_structure, d_conditioning, n_c_layers, k_c, r_c
        )
        self.bytenet = ConditionedByteNetDecoder(
            n_tokens, d_embedding, d_conditioning, d_model, n_layers, k_b, r_b
        )
