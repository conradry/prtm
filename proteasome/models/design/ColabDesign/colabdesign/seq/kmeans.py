...
def _kmeans(X, X_weight, n_clusters=8, n_init=10, max_iter=300, tol=1e-4, seed=0):
    ...
    def single_run(key):
        ...
        if tol == 0:
            means = jax.lax.scan(
                lambda mu, _: (_M(_E(mu)), None), init_means, None, length=max_iter
            )[0]
        else:
        ...
        return {"labels": _E(means), "means": means, "inertia": _inertia(means)}
...
def kmeans(x, x_weights, k, seed=0, max_iter=300):
    ...
    if k == 1:
        kms = {
            "means": (x * x_weights[:, None, None]).sum(0, keepdims=True)
            / x_weights.sum(),
        ...
        kms = _kmeans(
            x.reshape(N, -1), x_weights, n_clusters=k, max_iter=max_iter, seed=seed
        )
...
def kmeans_sample(msa, msa_weights, k=1, samples=None, seed=0):
    ...
    if samples is None:
        ...
        sampled_labels = jnp.sort(
            jax.random.choice(key_, jnp.arange(k), shape=(samples,), p=kms["cat"])
        )

    # sample MSA
    sampled_msa = kms["means"][sampled_labels]
    sampled_msa = (
        sampled_msa.cumsum(-1) >= jax.random.uniform(key, shape=(samples, L, 1))
    ).argmax(-1)
    o = {"kms": kms, "sampled_labels": sampled_labels, "sampled_msa": sampled_msa}
