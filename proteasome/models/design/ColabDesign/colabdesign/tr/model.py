...
from colabdesign.shared.utils import Key, copy_dict, dict_to_str, update_dict
...
class mk_tr_model(design_model):
        ...
        **kwargs,
        ...
        if self.protocol in ["fixbb", "partial"]:
            ...
            if self.protocol == "partial" and pos is not None:
                ...
                self._inputs["batch"] = jax.tree_map(
                    lambda x: x[p], self._inputs["batch"]
                )
                self.opt["pos"] = p
            ...
            self._inputs["6D"] = _np_get_6D_binned(
                self._inputs["batch"]["all_atom_positions"],
        ...
        if self.protocol in ["hallucination", "partial"]:
            ...
            for n in range(1, 6):
                model_params = get_model_params(
                    os.path.join(self._data_dir, f"/bkgr0{n}.npy")
                )
                self._inputs["6D_bkg"].append(
                    self._bkg_model(model_params, key, self._len)
                )
            self._inputs["6D_bkg"] = jax.tree_map(
                lambda *x: np.stack(x).mean(0), *self._inputs["6D_bkg"]
            )
    ...
    def step(self, backprop=True, callback=None, save_best=True, verbose=1):
        ...
        if save_best:
            if (
                "aux" not in self._tmp["best"]
                or self.aux["loss"] < self._tmp["best"]["aux"]["loss"]
            ):
