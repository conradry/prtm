...
from .utils import StructureDatasetPDB, _S_to_seq, parse_PDB, tied_featurize
...
class MPNN_wrapper:
    def __init__(self, model_name="v_48_020", verbose=False):
        ...
        backbone_noise = (
            0.00  # Standard deviation of Gaussian noise to add to backbone atoms
        )
        hidden_dim = 128
        num_layers = 3
    ...
    def prep_inputs(
        self, pdb_path, target_chain, fixed_chain=None, ishomomer=False, omit_AAs="X"
    ):
    ...
    def score(self, inputs, seq=None, order=None, key=None, unconditional=False):
        ...
        (
            ...
            batch_clones,
            inputs["chain_id_dict"],
            inputs["fixed_positions_dict"],
            inputs["omit_AA_dict"],
            inputs["tied_positions_dict"],
            inputs["pssm_dict"],
            inputs["bias_by_res_dict"],
        )
        score_input = {
            "X": X,
            "S": S,
    ...
    def sampling(
        self, inputs, sample_num, batch_size, sampling_temp=0.1, order=None, key=None
    ):
        ...
        pssm_log_odds_mask = jax.lax.convert_element_type(
            (pssm_log_odds_all > inputs["pssm_threshold"]), jnp.float32
        ...
        for _ in tqdm(range(NUM_BATCHES)):
            ...
            if inputs["tied_positions_dict"] is None:
                sample_dict = self.model.sample(
                    self.model.params, used_key.get(), sample_input
                )
            else:
                sample_input.update(
                    {
