...
from .utils import cat_neighbors_nodes, gather_nodes, get_ar_mask, scatter
...
class mpnn_sample:
    def sample(
        ...
        decoding_order = jnp.argsort(
            (chain_mask + 0.0001) * (jnp.abs(randn))
        ...
        for t_ in range(N_nodes):
            ...
            bias_by_res_gathered = jnp.take_along_axis(
                bias_by_res, jnp.tile(t[:, None, None], [1, 1, 21]), 1
            )[
                :, 0, :
            ]  # [B, 21]
            if jnp.equal(chain_mask_gathered, 0).all():
                ...
                # Hidden layers
                E_idx_t = jnp.take_along_axis(
                    E_idx, jnp.tile(t[:, None, None], [1, 1, E_idx.shape[-1]]), 1
                )
                h_E_t = jnp.take_along_axis(
                    h_E,
                    jnp.tile(
                        t[:, None, None, None], [1, 1, h_E.shape[-2], h_E.shape[-1]]
                    ),
                    1,
                )
                ...
                for l, layer in enumerate(self.decoder_layers):
                    ...
                    h_V_t = jnp.take_along_axis(
                        h_V_stack[l],
                        jnp.tile(t[:, None, None], [1, 1, h_V_stack[l].shape[-1]]),
                        1,
                    )
                    h_ESV_t = (
                        jnp.take_along_axis(
                    ...
                    h_V_stack[l + 1] = scatter(
                        h_V_stack[l + 1],
                        1,
                        jnp.tile(t[:, None, None], [1, 1, h_V.shape[-1]]),
                        layer(h_V_t, h_ESV_t, mask_V=mask_t),
                    )
                # Sampling step
                h_V_t = jnp.take_along_axis(
                    h_V_stack[-1],
                    jnp.tile(t[:, None, None], [1, 1, h_V_stack[-1].shape[-1]]),
                    1,
                )[:, 0]
                ...
                if pssm_bias_flag:
                    pssm_coef_gathered = jnp.take_along_axis(pssm_coef, t[:, None], 1)[
                        :, 0
                    ]
                    pssm_bias_gathered = jnp.take_along_axis(
                        pssm_bias,
                        jnp.tile(t[:, None, None], [1, 1, pssm_bias.shape[-1]]),
                        1,
                    )[:, 0]
                    probs = (
                ...
                if pssm_log_odds_flag:
                    pssm_log_odds_mask_gathered = jnp.take_along_axis(
                        pssm_log_odds_mask,
                        jnp.tile(
                            t[:, None, None], [1, 1, pssm_log_odds_mask.shape[-1]]
                        ),
                        1,
                    )[
                        :, 0
                    ...
                    probs = probs_masked / jnp.sum(
                        probs_masked, axis=-1, keepdims=True
                    )  # [B, 21]
                if omit_AA_mask_flag:
                    omit_AA_mask_gathered = jnp.take_along_axis(
                        omit_AA_mask,
                        jnp.tile(t[:, None, None], [1, 1, omit_AA_mask.shape[-1]]),
                        1,
                    )[
                        :, 0
                    ]  # [B, 21]
                ...
                S_t = jax.vmap(
                    lambda key, input, prob: jax.random.choice(key, input, p=prob),
                    in_axes=(0, 0, 0),
                    out_axes=0,
                )(used_key, input, probs)
                all_probs = scatter(
                    all_probs,
                    1,
                    jnp.tile(t[:, None, None], [1, 1, 21]),
                    (
                        chain_mask_gathered[
                            :,
                            :,
            ...
            temp1 = self.W_s(S_t)
            h_S = scatter(
                h_S, 1, jnp.tile(t[:, None, None], [1, 1, temp1.shape[-1]]), temp1
            )
    ...
    def tied_sample(
        ...
        decoding_order = jnp.tile(
            jnp.array(list(itertools.chain(*new_decoding_order)))[None,],
            [X.shape[0], 1],
        )
        ...
        for t_list in new_decoding_order:
                ...
                if (chain_mask[:, t] == 0).all():
                    ...
                    for l, layer in enumerate(self.decoder_layers):
                        ...
                        h_V_stack[l + 1] = (
                            h_V_stack[l + 1]
                            .at[:, t, :]
                            .set(layer(h_V_t, h_ESV_t, mask_V=mask_t).squeeze(1))
                        )
            ...
            if done_flag:
                ...
                if pssm_log_odds_flag:
                    ...
                    probs_masked += probs * 0.001
                    probs = probs_masked / jnp.sum(
                        probs_masked, aixs=-1, keepdims=True
                    )  # [B, 21]
                if omit_AA_mask_flag:
