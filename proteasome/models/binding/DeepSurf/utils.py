...
def simplify_dms(init_surf_file, factor, seed=None):
    ...
    kmeans = KMeans(n_clusters=nCl, max_iter=300, n_init=1, random_state=seed).fit(
        coords
    )
    ...
    for cl in cluster_idx:
        ...
        closest_idx_to_center = np.argmin(
            [euclidean(centers[cl], coords[idx]) for idx in cluster_points_idxs]
        )
