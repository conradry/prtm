...
from tensorflow.python.ops import math_ops, nn_ops, variable_scope
...
def bottleneck(
    ...
    with variable_scope.variable_scope(scope, "resid_v1", [inputs]) as sc:
        ...
        residual1 = resnet_3d_utils.conv3d_same(
            ...
            with arg_scope([layers.batch_norm], is_training=is_training):
                ...
                if global_pool:
                    net = math_ops.reduce_mean(
                        net, [1, 2, 3], name="pool5", keepdims=True
                    )
...
def resnet_block(scope, depth_out, num_units, stride):
    """Args:
    ...
    """
    return resnet_3d_utils.Block(
        scope,
        bottleneck,
        [{"depth_out": depth_out, "depth_bottleneck": depth_out / 4, "stride": 1}]
        * (num_units - 1)
        + [
            {
                "depth_out": depth_out,
                "depth_bottleneck": depth_out / 4,
                "stride": stride,
            }
        ],
    )
...
def resnet_v1_18(
    ...
    blocks = [
        resnet_block("block1", depth_out=64, num_units=2, stride=2),
        resnet_block("block2", depth_out=128, num_units=2, stride=2),
        resnet_block("block3", depth_out=256, num_units=2, stride=2),
        resnet_block("block4", depth_out=512, num_units=2, stride=1),
    ]
    return resnet_v1(
        inputs,
        blocks,
