...
from run_alphafold_stage2a_comp import (FLAGS, MAX_MSA_DEPTH_MONO,
                                        _read_target_file)
...
def _get_feature_dict(target, target_name):
    ...
    for seq_name in seq_names:
        ...
        with open(features_input_path, "rb") as f:
            ...
            print(
                f"Info: {target_name} found monomer {seq_name} msa_depth = {N}, seq_len = {L}, num_templ = {T}"
            )
            if N > MAX_MSA_DEPTH_MONO:
                print(
                    f"Info: {target_name} MSA size is too large, reducing to {MAX_MSA_DEPTH_MONO}"
                )
...
def main(argv):
    ...
    for target in target_lst:
        ...
        for pkl_file in os.listdir(target_dir):
            if (
                ".pkl" in pkl_file
                and pkl_file.startswith("model_")
                and "_ptm_" in pkl_file
            ):
