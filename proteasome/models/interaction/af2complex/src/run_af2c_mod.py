...
flags.DEFINE_list(
    "model_names",
    None,
    "Names of deep learning models to use. This is required even with model_preset option.",
)
...
flags.DEFINE_integer(
    "max_recycles", None, "The maximum number of recycles.", lower_bound=1
)
flags.DEFINE_float(
    "recycle_tol",
    None,
    "The tolerance for recycling, caculated as the RMSD change "
    "in the distogram of backbone Ca atoms. Recycling stops "
    "if the change is smaller than this value",
    lower_bound=0.0,
    upper_bound=2.0,
)
flags.DEFINE_integer(
    "num_ensemble",
    None,
    "The number of ensembles of each model, 1 means no ensembling.",
    lower_bound=1,
)
flags.DEFINE_integer(
    "max_msa_clusters", None, "The maximum number of MSA clusters.", lower_bound=1
)
flags.DEFINE_integer(
    "max_extra_msa", None, "The maximum number of extra MSA clusters.", lower_bound=1
)
flags.DEFINE_boolean(
    "write_complex_features",
    False,
    "Save the feature dict for "
...
flags.DEFINE_boolean(
    "output_pickle",
    True,
    "Write the prediction results into a pickle file. "
    "Note that the pickle files are quite large. Disable it to save disk space.",
)
flags.DEFINE_integer(
    "save_recycled",
    0,
    "0 - no recycle info saving, 1 - print "
    ...
    "dictionaries of each recycling iteration.",
    lower_bound=0,
    upper_bound=3,
)
flags.DEFINE_string(
    "checkpoint_tag",
    None,
    "Enable checkpoint and use the tag to name "
...
flags.DEFINE_integer(
    "mono_msa_crop_size",
    5000,
    "For monomer MSA cropping in the multimer_np mode",
    lower_bound=1,
)
flags.DEFINE_integer(
...
flags.DEFINE_boolean(
    "do_cluster_analysis",
    False,
    "Whether to print out clusters of protein chains in the prediction",
)
flags.DEFINE_integer(
    "cluster_edge_thres",
    10,
    "The number of contacts between chains that constitute an edge in the "
    "cluster analysis",
    lower_bound=0,
)
flags.DEFINE_float(
    "pdb_iscore_cf",
    -1.0,
    "If interface icore is present, only write the pdb of the structural model "
    "if the iScore is larger than this cutoff value. Useful for large-scale screening. ",
)
flags.DEFINE_boolean(
    "allow_dropout",
    False,
    "Allow dropout during model inference. This is an experimental feature. "
    "Default is disabled.",
)
...
def load_checkpoint_file(ckpt_file_path: str):
    with open(ckpt_file_path, "rb") as f:
        ...
        f.close()
        logging.info(
            f"Previous checkpoint was saved at recycle {prev_ckpt_iter}, continuing..."
        )
...
def predict_structure(
    ...
    print(
        f"Info: modeling {target_name} with msa_depth = {N}, seq_len = {L}, num_templ = {T}"
    )
    ...
    ptms = {}
    pitms = {}  # predicted TM-score
    ires = {}
    icnt = {}  # interfacial residues and contacts
    tols = (
        {}
    )  # change in backbone pairwise distance to check with the recyle tolerance criterion
    ...
    for model_index, (model_name, model_runner) in enumerate(model_runners.items()):
        ...
        if flags.checkpoint_tag:
            ...
            model_runner.checkpoint_file = os.path.join(
                checkpoint_dir, model_name + "_" + flags.checkpoint_tag + ".pkl"
            )
            if os.path.exists(model_runner.checkpoint_file):
                logging.info(
                    f"Found checkpoint file {model_runner.checkpoint_file}, loading..."
                )
                prev_ckpt, prev_ckpt_iter = load_checkpoint_file(
                    model_runner.checkpoint_file
                )
        ...
        if (
            max_msa_clusters is not None
            and max_extra_msa is not None
            and not is_multimer
        ):
            msa_clusters = max(min(N, max_msa_clusters), 5)
            model_runner.config.data.eval.max_msa_clusters = msa_clusters
            model_runner.config.data.common.max_extra_msa = max(
                min(N - msa_clusters, max_extra_msa), 1
            )
        if flags.preset in ["genome", "genome2", "super"]:
            ...
            if (
                L > 1180
            ):  ### memory limit of a single 16GB GPU, applied to cases with multiple ensembles
                num_en = 1
            else:
                num_en = num_ensemble
            print(
                f"Info: {target_name} reset max_recycles = {max_iter}, num_ensemble = {num_en}"
            )
            if not is_multimer:
        ...
        t_0 = time.time()
        prediction_result, (tot_recycle, tol_value, recycled) = model_runner.predict(
        ...
        if prev_ckpt_iter:
            print(f"Info: total recycle number is {tot_recycle}")
        ...
        iterations[
            model_out_name
        ] = tot_recycle.tolist()  ### convert jax numpy to regular list for json saving

        t_diff = time.time() - t_0
        timings[f"predict_and_compile_{model_out_name}"] = round(t_diff, 1)
        logging.info(
            "Total JAX model %s predict time (includes compilation time): %.1f seconds",
            model_out_name,
            t_diff,
        )
        ...
        def _save_results(result, log_model_name, out_dir, recycle_index):
            ...
            if recycle_index < tot_recycle:
                ...
                print(
                    f"Info: {target_name} {log_model_name}, ",
                    f"tol = {tol:5.2f}, pLDDT = {plddt:.2f}, pTM-score = {ptm:.4f}",
                    end="",
                )
                if (
                    len(monomers) > 1 or monomers[0]["copy_number"] > 1
                ):  # hetero- or homo-oligomer target
                    print(
                        f", piTM-score = {pitm:.4f}, interface-score = {inter_sc:.4f}",
                ...
                print(
                    f"Info: {target_name} {log_model_name} performed {tot_recycle} recycles,",
                    f"final tol = {tol_value:.2f}, pLDDT = {plddt:.2f}",
                    end="",
                )
                if "iptm+ptm" in result:
                    ...
                    if "cluster_analysis" in result:
                        ...
                        for cluster in clus_res["clusters"]:
                            cluster_identities.append(
                                [idx2chain_name[c] for c in cluster]
                            )
                        clus[log_model_name] = {
            ...
            if (
                (recycle_index == tot_recycle and flags.output_pickle)
                or FLAGS.save_recycled == 3
            ) and inter_sc > FLAGS.pdb_iscore_cf:
                result_output_path = os.path.join(out_dir, f"{log_model_name}.pkl")
                with open(result_output_path, "wb") as f:
                    pickle.dump(result, f, protocol=4)

            if (
                recycle_index == tot_recycle or FLAGS.save_recycled >= 2
            ) and inter_sc > FLAGS.pdb_iscore_cf:
        ...
        if FLAGS.save_recycled:
            ...
            if FLAGS.save_recycled > 1 and not os.path.exists(recycle_out_dir):
                try:
                    ...
                    print(
                        f"Warning: tried to create an existing {recycle_out_dir}, ignored"
                    )

            for rec_idx, rec_dict in enumerate(recycled):
                if prev_ckpt_iter:
                    rec_idx += prev_ckpt_iter
                if rec_idx < tot_recycle:
                    _save_results(
                        rec_dict,
                        f"{model_out_name}_recycled_{rec_idx:02d}",
                        recycle_out_dir,
                        rec_idx,
                    )
    ...
    if len(pitms):  ## extra metrics for evaluating interactions
        stats = {
            **stats,
            "pitms": pitms,
            "interfacial residue number": ires,
            "interficial contact number": icnt,
            "interface score": ints,
            "iptm+ptm": iptms,
            "clusters": clus,
        }
    ...
    if len(model_runners) > 1:  # more than 1 model
        ranking_output_path = os.path.join(
            output_dir, "ranking_all" + out_suffix + ".json"
        )
    else:  # only one model, use different model names to avoid overwriting same file
        ranking_output_path = os.path.join(
            output_dir, "ranking_" + model_out_name + ".json"
        )
...
def main(argv):
    ...
    if FLAGS.max_msa_clusters is not None and FLAGS.max_extra_msa is not None:
        ...
        print(
            f"Info: max_msa_clusters = {max_msa_clusters}, max_extra_msa = {max_extra_msa}"
        )
    ...
    def make_model_runners(model_names, model_preset):
        ...
        for model_name in model_names:
            ...
            if "monomer" in model_preset and "multimer" in model_name:
                raise ValueError(
                    f"{model_name} is not compatible with {FLAGS.model_preset}"
                )
    ...
    time.sleep(
        random.randint(0, 10)
    )  # mitigating creating the same output directory from multiple runs
    ...
    for target in target_lst:
        ...
        if (
            FLAGS.model_names != model_names or FLAGS.model_preset != model_preset
        ) and (model_names is not None and model_preset is not None):
        ...
        if target["adj_list_path"] is not None and FLAGS.msa_pairing != "custom":
            print(
                f"WARNING: adjacency list provided for {target_name}, "
