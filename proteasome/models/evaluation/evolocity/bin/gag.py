...
def parse_args():
    ...
    parser.add_argument(
        "model_name", type=str, help="Type of language model (e.g., esm1b, tape)"
    )
    parser.add_argument("--namespace", type=str, default="gag", help="Model namespace")
...
def gag_siv_cpz(args, model, seqs, vocabulary, namespace="glo"):
    ...
    df = pd.DataFrame(
        dist_data, columns=["gag_type", "name", "score", "homology", "host"]
    )
...
def evo_gag(args, model, seqs, vocabulary, namespace="gag"):
    ...
    adata.obs["simple_subtype"] = [
        subtype if subtype in keep_subtypes else ("A" if "A" in subtype else "Other")
    ...
    tprint(
        "Pseudotime-time Pearson r = {}, P = {}".format(
            *ss.pearsonr(adata.obs["pseudotime"][nnan_idx], adata.obs["year"][nnan_idx])
        )
    )
    ...
    if args.model_name != "tape":
        nnan_idx = np.isfinite(adata.obs["homology"]) & np.isfinite(
