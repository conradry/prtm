...
def parse_args():
    ...
    parser.add_argument("--namespace", type=str, default="cyc", help="Model namespace")
    ...
    for idx, (name, seq) in enumerate(nodes):
        for uniprot_seq in seqs:
            ...
            if tax_type not in tax_types:
                continue
            if tax_type == "fungi" and ("all" not in name and "fungi" not in name):
                continue
...
def evo_cyc(args, model, seqs, vocabulary, namespace="cyc"):
    ...
    if "_onehot" in namespace:
        ...
        sc.pp.neighbors(adata, n_neighbors=30, metric="manhattan", use_rep="X_onehot")
        sc.tl.umap(adata)
