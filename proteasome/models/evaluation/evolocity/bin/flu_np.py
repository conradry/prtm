...
def parse_args():
    ...
    parser.add_argument("--namespace", type=str, default="np", help="Model namespace")
...
def split_seqs(seqs, split_method="random"):
    ...
    for seq in seqs:
        ...
        seq_dates = [meta["year"] for meta in seqs[seq] if meta["year"] is not None]
        if len(seq_dates) == 0:
...
def setup(args):
    ...
    model = get_model(args, seq_len, vocab_size, inference_batch_size=1000)
...
def interpret_clusters(adata):
    ...
    for cluster in cluster2subtype:
        ...
        tprint("\tCluster {}, largest subtype % = {}".format(cluster, pct_subtype))
...
def plot_umap(adata, namespace="np"):
    sc.pl.umap(
        adata,
        color="year",
        save=f"_{namespace}_year.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="louvain",
        save=f"_{namespace}_louvain.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="subtype",
        save=f"_{namespace}_subtype.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="simple_subtype",
        save=f"_{namespace}_simple_subtype.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="host",
        save=f"_{namespace}_host.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="resist_adamantane",
        save=f"_{namespace}_adamantane.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="resist_oseltamivir",
    ...
    sc.pl.umap(
        adata,
        color="virulence",
        save=f"_{namespace}_virulence.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="transmission",
        save=f"_{namespace}_transmission.png",
        edges=True,
    )
    sc.pl.umap(
        adata,
        color="homology",
        save=f"_{namespace}_homology.png",
        edges=True,
    )
        ...
        for i in tqdm(range(n_obs)):
            ...
            for j in neighs_idx:
                val = score_fn(
                    ...
                    vocabulary,
                    model,
                    seq_cache=vgraph.seq_probs,
                    verbose=vgraph.verbose,
                )
                fields = [i, j, adata.obs["year"][i], adata.obs["year"][j], val]
...
def epi_gong2013(args, model, seqs, vocabulary, namespace="np"):
    ...
    if namespace == "np":
        ...
        for idx, (name, seq) in enumerate(nodes):
            if idx > 0:
                ...
                data.append([name, seq, score_full, score_muts, score_self])
                tprint(
                    "{}: {}, {}, {}".format(name, score_full, score_muts, score_self)
                )

        df = pd.DataFrame(data, columns=["name", "seq", "full", "muts", "self_score"])
        ...
        tprint(
            "Gong et al. Spearman r = {}, P = {}".format(*ss.spearmanr(gong_x, gong_y))
        )
    ...
    if "_onehot" in namespace:
        ...
        sc.pp.neighbors(adata, n_neighbors=40, metric="manhattan", use_rep="X_onehot")
        sc.tl.umap(adata)
    ...
    nnan_idx = np.isfinite(adata.obs["year"]) & np.isfinite(adata.obs["pseudotime"])
    ...
    tprint(
        "Pseudotime-time Spearman r = {}, P = {}".format(
            *ss.spearmanr(
                adata_nnan.obs["pseudotime"], adata_nnan.obs["year"], nan_policy="omit"
            )
        )
    )
    tprint(
        "Pseudotime-time Pearson r = {}, P = {}".format(
            *ss.pearsonr(adata_nnan.obs["pseudotime"], adata_nnan.obs["year"])
        )
    )
    ...
    tprint(
        "Pseudotime-length Pearson r = {}, P = {}".format(
            *ss.pearsonr(adata_nnan.obs["pseudotime"], adata_nnan.obs["seqlen"])
        )
    )
