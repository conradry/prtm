...
def benchmark_temporal(protein, setting, percentage, seed, label):
    ...
    return ss.spearmanr(adata.obs[label], adata.obs["pseudotime"], nan_policy="omit")[
        0
    ], len(adata)
...
def benchmark_class(protein, setting, percentage, seed, labels):
    ...
    if len(set(classes)) > 2:
        return (
            ss.spearmanr(classes, pseudotimes, nan_policy="omit")[0],
            "spearmanr",
            len(adata),
        )
...
if __name__ == "__main__":
    ...
    for setting in settings:
        ...
        df_setting["value"] = [
            ...
            for value, score_type in zip(df_setting["value"], df_setting["score_type"])
        ]
