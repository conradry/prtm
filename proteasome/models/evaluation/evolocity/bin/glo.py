...
def parse_args():
    ...
    parser.add_argument("--namespace", type=str, default="glo", help="Model namespace")
    parser.add_argument("--dim", type=int, default=512, help="Embedding dimension")
    parser.add_argument("--seed", type=int, default=1, help="Random seed")
    parser.add_argument(
        "--ancestral", action="store_true", help="Analyze ancestral sequences"
    )
...
def parse_meta(record, taxonomy):
    if "GN=" in record:
        ...
        (_, accession, gene_id, species, species_id, pe, sv) = record.split("|")
        gene_symbol = None
    ...
    elif "alpha" in gene_id or "hba" in gene_id:
        globin_type = "hemoglobin_alpha"
    elif "hbb" in gene_id or "beta" in gene_id:
...
def plot_umap(adata, namespace="glo"):
    sc.pl.umap(
        adata, color="tax_group", edges=True, save="_{}_taxonomy.png".format(namespace)
    )
    sc.pl.umap(
        adata, color="globin_type", edges=True, save="_{}_glotype.png".format(namespace)
    )
    sc.pl.umap(
        adata, color="louvain", edges=True, save="_{}_louvain.png".format(namespace)
    )
    sc.pl.umap(
        adata, color="seq_len", edges=True, save="_{}_seqlen.png".format(namespace)
    )
    sc.pl.umap(
...
def globin_ancestral(args, model, seqs, vocabulary, namespace="glo"):
    ...
    nodes = [
        (record.id, str(record.seq)) for record in SeqIO.parse(path_fname, "fasta")
    ]
    ...
    glo_types = {"myoglobin", "hemoglobin_alpha", "hemoglobin_beta"}
    ...
    df = pd.DataFrame(dist_data, columns=["glo_type", "name", "score", "homology"])
...
def globin_paths(path_fname, args, model, seqs, vocabulary, namespace="glo"):
    ...
    for idx, (name, seq) in enumerate(nodes):
        ...
        tprint("{}: {}, {}, {}".format(name, score_full, score_muts, score_self))
...
def evo_globin(args, model, seqs, vocabulary, namespace="glo"):
    ...
    if args.downsample < 100:
        ...
        subtype_weights = {
            ...
            "cytoglobin": 1.0,
            "myoglobin": 1.0,
            "hemoglobin_alpha": 2.0,
            "hemoglobin_beta": 2.0,
            "other/unlabeled": 1.0,
        }
        weights = np.array(
            [subtype_weights[subtype] for subtype in adata.obs["globin_type"]]
        )
        weights /= sum(weights)
    ...
    evo.pl.velocity_contour(
        ...
        c="#aaaaaa",
        show=False,
        save=f"_{namespace}_contour.png",
        dpi=500,
    )
    ...
    nnan_idx = np.isfinite(adata.obs["homology"]) & np.isfinite(adata.obs["pseudotime"])
    tprint(
        "Pseudotime-homology Spearman r = {}, P = {}".format(
            *ss.spearmanr(
                adata.obs["pseudotime"][nnan_idx],
    ...
    tprint(
        "Pseudotime-homology Pearson r = {}, P = {}".format(
            *ss.pearsonr(
                adata.obs["pseudotime"][nnan_idx], adata.obs["homology"][nnan_idx]
            )
        )
    )
    ...
    tprint(
        "Pseudotime-length Spearman r = {}, P = {}".format(
            *ss.spearmanr(adata.obs["pseudotime"], seqlens, nan_policy="omit")
        )
    )
