...
class Model:
    ...
    def build(self):
        ...
        with tf.name_scope("input"):
            ...
            # 2D convolution part
            tbt_in = tf.placeholder(
                tf.float32, shape=[self.prot_size, self.prot_size, self.tbt_size]
            )
            ...
            # Target values
            estogram_in = tf.placeholder(
                tf.float32, shape=[self.prot_size, self.prot_size, 15]
            )
        ...
        with tf.name_scope("3d_conv"):
            if not self.ignore3dconv:
                ...
                layers.append(
                    tf.layers.conv3d(grid3d, 20, 1, padding="same", use_bias=False)
                )

                # 1st conv3d & batch_norm & droput & activation
                layers.append(
                    tf.layers.conv3d(layers[-1], 20, 3, padding="valid", use_bias=True)
                )
                layers.append(
                    tf.keras.layers.Dropout(rate=dropout_rate)(
                        layers[-1], training=isTraining
                    )
                )
                ...
                layers.append(
                    tf.layers.conv3d(layers[-1], 30, 4, padding="valid", use_bias=True)
                )
                layers.append(tf.nn.elu(layers[-1]))

                # 3rd conv3d & batch_norm & activation
                layers.append(
                    tf.layers.conv3d(layers[-1], 20, 4, padding="valid", use_bias=True)
                )
                layers.append(tf.nn.elu(layers[-1]))
                ...
                layers.append(
                    tf.layers.average_pooling3d(
                        layers[-1], pool_size=4, strides=4, padding="valid"
                    )
                )
        ...
        with tf.name_scope("2d_conv"):
            ...
            layers.append(
                tf.nn.elu(tf.layers.conv1d(layers[-1], 60, 1, padding="SAME"))
            )

            # Put them together with tbt with self.tbt_size
            tbt = tf.concat(
                [
                    tf.tile(layers[-1][:, :, None, :], [1, 1, nres, 1]),
            ...
            if self.symmetrize:
                error_predictor = 0.5 * (
                    error_predictor + tf.transpose(error_predictor, perm=[0, 2, 1, 3])
                )
            logits_error = tf.layers.conv2d(
                error_predictor, filters=15, kernel_size=(1, 1)
            )
            estogram_predicted = tf.nn.softmax(logits_error)[0]

            # Resnet prediction for errorgram branch
            mask_predictor = build_resnet(
                _input=layers[-1],
                channel=128,
                num_chunks=1,
                require_in=False,
            ...
            mask_predictor = 0.5 * (
                mask_predictor + tf.transpose(mask_predictor, perm=[0, 2, 1, 3])
            )
            logits_mask = tf.layers.conv2d(
                mask_predictor, filters=1, kernel_size=(1, 1)
            )[:, :, :, 0]
        ...
        with tf.name_scope("cost"):
            ...
            if self.verbose:
                print("Check 1:", estogram.shape, logits_error.shape)
            estogram_entropy = tf.nn.softmax_cross_entropy_with_logits_v2(
                labels=estogram, logits=logits_error, axis=-1
            )

            # Auxiliary estogram
            if self.verbose:
                print("Check 2:", mask.shape, logits_mask.shape)
            mask_entropy = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=mask, logits=logits_mask
            )

            # Lddt evaluation via MSE
            if self.verbose:
                print("Check 3:", lddt.shape, lddt_predicted.shape)
        ...
        with tf.control_dependencies(update_ops):
            ...
            clipped = [
                (tf.clip_by_value(grad, -5, 5), tvar) for grad, tvar in grads_and_vars
            ]
    ...
    def calculate_LDDT(self, estogram, mask, center=7):
        with tf.name_scope("lddt"):
            ...
            mask = tf.multiply(
                mask, tf.ones(tf.shape(mask)) - tf.eye(tf.shape(mask)[0])
            )
            masked = tf.transpose(
                tf.multiply(tf.transpose(estogram, [2, 0, 1]), mask), [1, 2, 0]
            )

            p0 = tf.reduce_sum(masked[:, :, center], axis=-1)
            p1 = tf.reduce_sum(
                masked[:, :, center - 1] + masked[:, :, center + 1], axis=-1
            )
            p2 = tf.reduce_sum(
                masked[:, :, center - 2] + masked[:, :, center + 2], axis=-1
            )
            p3 = tf.reduce_sum(
                masked[:, :, center - 3] + masked[:, :, center + 3], axis=-1
            )
    ...
    def train(
        ...
        while self.e < epochs:
            ...
            for i in range(len(X.proteins)):
                ...
                if i % 50 == 49:
                    sys.stdout.write(
                        "Epoch: [%2d/%2d], Batch: [%2d/%2d], loss: %.2f, esto-loss: %.2f, lddt-loss: %.2f\n"
                        % (
                            self.e,
                            epochs,
                            i,
                            len(X.proteins),
                            np.mean(train_cost),
                            np.mean(train_e_cost),
                            np.mean(train_l_cost),
                        )
                    )
                    train_cost = list()
                    train_e_cost = list()
            ...
            if V != None:
                ...
                for i in range(len(V.proteins)):
                    ...
                    cost, e_cost, l_cost, m_cost = self.sesh.run(
                        ops_to_run, feed_dict=feed_dict
                    )
                ...
                sys.stdout.write(
                    "Valid: [%2d/%2d], Batch: [%2d/%2d], loss: %.2f, esto-loss: %.2f, lddt-loss: %.2f\n"
                    % (
                        self.e,
                        epochs,
                        i,
                        len(V.proteins),
                        np.mean(valid_cost),
                        np.mean(valid_e_cost),
                        np.mean(valid_l_cost),
                    )
                )
            ...
            if self.name != None:
                ...
                if save_best:
                    ...
                    if V is None:
                        ...
                        return
                    if len(self.loss["valid"]) == 1 or (
                        self.loss["valid"][-1] < np.min(self.loss["valid"][:-1])
                    ):
    ...
    def predict(self, batch):
        ...
        operations2 = [self.ops["lddt"], self.ops["estogram"], self.ops["mask"]]

        return (
            self.sesh.run(operations, feed_dict=feed_dict),
            self.sesh.run(operations2, feed_dict=feed_dict),
        )
    ...
    def predict2(self, batch):
        ...
        if self.ignore3dconv:
            feed_dict = {self.ops["obt"]: f1d, self.ops["tbt"]: f2d}
        else:
            feed_dict = {
                self.ops["obt"]: f1d,
                self.ops["tbt"]: f2d,
