...
def pixelSelfAttention(x, maxpool=1, kq_factor=8, v_factor=2, reuse=False):
    # Get parameters
    ...
    # Key
    f = tf.layers.conv2d(inputs=x, filters=c // kq_factor, kernel_size=1, strides=1)

    if maxpool > 1:
        f = tf.layers.max_pooling2d(
            f, pool_size=maxpool, strides=maxpool, padding="SAME"
        )
    # Query
    g = tf.layers.conv2d(inputs=x, filters=c // kq_factor, kernel_size=1, strides=1)

    # Value
    h = tf.layers.conv2d(inputs=x, filters=c // v_factor, kernel_size=1, strides=1)

    if maxpool > 1:
        h = tf.layers.max_pooling2d(
            h, pool_size=maxpool, strides=maxpool, padding="SAME"
        )
    print(h)
    ...
    attent_map = tf.matmul(hw_flatten(g), hw_flatten(f), transpose_b=True)
    ...
    o = tf.layers.conv2d(inputs=o, filters=c, kernel_size=1, strides=1)
