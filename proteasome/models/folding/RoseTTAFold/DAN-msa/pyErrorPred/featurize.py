...
init(
    extra_options="-constant_seed -mute all -multithreading:interaction_graph_threads 1 -multithreading:total_threads 1"
)
...
def get_hbonds(pose):
    ...
    hbond_set = (
        pose.energies()
        .data()
        .get(pyrosetta.rosetta.core.scoring.EnergiesCacheableDataType.HBOND_SET)
    )
    for i in range(1, hbond_set.nhbonds()):
        ...
        if hb:
            acceptor = hb.acc_res()
            donor = hb.don_res()
            wtype = pyrosetta.rosetta.core.scoring.hbonds.get_hbond_weight_type(
                hb.eval_type()
            )
            energy = hb.energy()
...
def extract_EnergyDistM(pose, energy_terms):
    ...
    for i in range(length):
        ...
        while iru != irue:
            ...
            for k in range(len(evals)):
                ...
                if (
                    t == pyrosetta.rosetta.core.scoring.ScoreType.hbond_bb_sc
                    or t == pyrosetta.rosetta.core.scoring.ScoreType.hbond_sc
                ):
                    if e != 0.0:
    ...
    for i in range(length):
        for j in range(length):
            ...
            tensor[
                0, index1 - 1, index2 - 1
            ] = distance  # 1-sigmoid(displacement, scale=10, offset=-5)
...
def set_neighbors3D(pdict):
    ...
    b = a[
        (np.min(a[:, 1:4], axis=-1) >= 0)
        & (np.max(a[:, 1:4], axis=-1) < nbins)
        & (a[:, 5] > 1e-5)
    ]
...
def set_features1D(pdict):
    ...
    dssp8 = np.array(list(DSSP.get_dssp_unreduced_secstruct()), dtype="|S1").view(
        np.uint8
    )
    ...
    # backbone (phi,psi)
    phi = np.array(np.deg2rad([p.phi(i) for i in range(1, nres + 1)])).astype(
        np.float32
    )
    psi = np.array(np.deg2rad([p.psi(i) for i in range(1, nres + 1)])).astype(
        np.float32
    )
...
def extractOneBodyTerms(pose, padval=0):
    ...
    return np.concatenate(
        [bond_angles_lengths_mat, energy_term_mat, SS_mat]
    ), features2 + score_terms + ["E", "L", "H"]
