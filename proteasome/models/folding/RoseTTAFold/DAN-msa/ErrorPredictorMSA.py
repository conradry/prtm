...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Error predictor network with predicted distogram", epilog="v0.0.1"
    )
    parser.add_argument(
        "distogram",
        action="store",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Roll predicted distogram or not [False], should be added if you use distogram from Trunk",
    )
    parser.add_argument(
        "--pdb",
        "-pdb",
    ...
    if len(args.outfolder) > 1:
        print(
            f"Only one output folder can be specified, but got {args.outfolder}",
            file=sys.stderr,
        )
        return -1
    ...
    if not args.pdb:
        ...
        if not (".pdb" in args.infolder and ".npz" in args.outfolder):
            print(
                "Input needs to be in .pdb format, and output needs to be in .npz format.",
                file=sys.stderr,
            )
            return -1
    ...
    if not args.pdb:
        if args.prefix == None:
            samples = [
                i[:-4]
                for i in os.listdir(args.infolder)
                if isfile(args.infolder + "/" + i) and i[-4:] == ".pdb" and i[0] != "."
            ]
            ignored = [
                i[:-4]
                for i in os.listdir(args.infolder)
                if not (
                    isfile(args.infolder + "/" + i) and i[-4:] == ".pdb" and i[0] != "."
                )
            ]
        else:
            samples = [
                i[:-4]
                for i in os.listdir(args.infolder)
                if isfile(args.infolder + "/" + i)
                and i[-4:] == ".pdb"
                and i[0] != "."
                and args.prefix in i
            ]
            ignored = [
                i[:-4]
                for i in os.listdir(args.infolder)
        ...
        if not args.reprocess:
            arguments = [
                (inputs[i], tmpoutputs[i], args.verbose)
                for i in range(len(inputs))
                if not isfile(tmpoutputs[i])
            ]
            already_processed = [
                (inputs[i], tmpoutputs[i], args.verbose)
                for i in range(len(inputs))
                if isfile(tmpoutputs[i])
            ]
            if args.verbose:
                print(
                    "Featurizing",
            ...
            arguments = [
                (inputs[i], tmpoutputs[i], args.verbose) for i in range(len(inputs))
            ]
            already_processed = [
                (inputs[i], tmpoutputs[i], args.verbose)
                for i in range(len(inputs))
        ...
        ###########################
        samples = [
            s for s in samples if isfile(join(args.outfolder, s + ".features.npz"))
        ]
        ...
        pyErrorPred.predict(
            samples, disto, modelpath, args.outfolder, verbose=args.verbose
        )
    ...
    # Processing for single sample
        ...
        if (not isfile(feature_file_name)) or args.reprocess:
            pyErrorPred.process(
                (join(infolder, insamplename + ".pdb"), feature_file_name, args.verbose)
            )
        if isfile(feature_file_name):
            ...
            if args.roll:
                disto = np.roll(disto, 1, axis=-1)
            pyErrorPred.predict(
                [outsamplename], disto, modelpath, outfolder, verbose=args.verbose
            )

            if not args.leavetemp:
                pyErrorPred.clean([outsamplename], outfolder, verbose=args.verbose)
        else:
