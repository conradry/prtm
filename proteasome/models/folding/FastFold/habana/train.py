...
from fastfold.habana.distributed import get_data_parallel_world_size, init_dist
...
def main():
    ...
    for epoch in range(200):
        ...
        for batch in train_dataloader:
            ...
            batch = {
                k: torch.as_tensor(v).to(device="hpu", non_blocking=True)
        ...
        if test_dataloader is not None:
            ...
            for batch in test_dataloader:
                batch = {
                    k: torch.as_tensor(v).to(device="hpu") for k, v in batch.items()
                }
                with torch.no_grad():
                    output = model(batch)
                    batch = tensor_tree_map(lambda t: t[..., -1], batch)
