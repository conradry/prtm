...
def download_trained_model(modeldir):
    print(
        "Downloading trained model (~140 MB) as first time setup to ",
        modeldir,
    ...
    for part in ["1", "2"]:
        request.urlretrieve(
            f"https://github.com/psipred/DMPfold2/raw/master/dmpfold/trained_model/FINAL_fullmap_e2e_model_part{part}.pt",
...
def aln_to_coords(
    ...
    if weights_file is None:
        ...
        if not os.path.isfile(
            os.path.join(modeldir, "FINAL_fullmap_e2e_model_part1.pt")
        ):
        ...
        trained_model.update(
            torch.load(
                os.path.join(modeldir, "FINAL_fullmap_e2e_model_part2.pt"),
                map_location=lambda storage, loc: storage,
            )
        )
    ...
    alnmat = (
        np.frombuffer(
            "".join(aln).translate(aa_trans).encode("latin-1"), dtype=np.uint8
        )
        - ord("A")
    ...
    f2d_dca = (
        fast_dca(msa1hot, w).float()
        if nseqs > 1
        else torch.zeros((length, length, 442), device=device)
    )
    f2d_dca = f2d_dca.permute(2, 0, 1).unsqueeze(0)

    if init_coords is not None:
        dmap = (
            (init_coords - init_coords.transpose(0, 1))
            .pow(2)
            .sum(dim=2)
            .sqrt()
            .unsqueeze(0)
...
def run_dmpfold():
    ...
    coords, confs, alnmat = aln_to_coords(
        args.input_file,
        device=args.device,
    ...
    for ri in range(coords.size(0)):
        for ai, an in enumerate(atoms):
            if alnmat[0, ri] != 7 or ai != 4:
                print(
                    "ATOM   %4d %s %s  %4d    %8.3f%8.3f%8.3f  1.00%6.2f"
                    % (
                        atomnum,
                        an,
                        rnamedict[alnmat[0, ri]],
                        ri + 1,
                        coords[ri, ai, 0].item(),
                        coords[ri, ai, 1].item(),
                        coords[ri, ai, 2].item(),
